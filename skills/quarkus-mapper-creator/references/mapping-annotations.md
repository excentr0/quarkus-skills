...
Compare entity fields (read from the entity source in Step 2 of SKILL.md) with DTO fields
(read from the DTO source).
...
Panache public fields are visible to MapStruct as fields; no accessors required.
...
## Panache entities and the id field

- Entity fields are `public` — MapStruct maps them directly. Never introduce
  getters/setters into the entity or the mapper to satisfy the mapper.
- `PanacheEntity` contributes an inherited `public Long id`; a DTO field named
  `id` maps automatically. Do not map the id into a flat association stub unless
  the user asked for it.
- If the entity is private-field style (getters/setters), MapStruct uses the
  accessors transparently — the `@Mapping` rules above do not change.