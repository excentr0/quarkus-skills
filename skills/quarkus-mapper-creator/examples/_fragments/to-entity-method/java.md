...
intermediate `new PetType()` and the field assignments.
...
| `${assocFieldName}` | entity association field (the ToOne side) | e.g. `type` |
| `${nestedFieldName}` | field on the association entity | e.g. `id`, `name` |
| `${inverseMethodName}` | name of toDto method | e.g. `toOrderDto` |

## Panache note

Panache entities declare public fields, so dot-notation and plain `@Mapping` need no
accessors: MapStruct assigns `pet.type = new PetType()` field-style in the generated code.
Do not add getters/setters to the entity for the mapper's sake. If the entity uses private
fields with accessors, MapStruct uses those transparently — the annotations above are
unchanged.

The inherited `public Long id` from `PanacheEntity` maps automatically for a same-named DTO
field; do not declare an `id` field in the entity.