...
```java
@org.mapstruct.Mapping(target = "${flatDtoFieldName}", expression = "java(${functionName}(${entityParamName}.${assocFieldName}))")
${dtoClassFqn} ${methodName}(${entityClassFqn} ${entityParamName});
```
...
- `${assocFieldName}` — the association field name on the entity (e.g. `specialties`, `pets`); Panache entities are read by direct field access, not by getter
...
| `${assocFieldName}` | entity association field for flat collection helpers | e.g. `pets` |

## Panache note

Flat collection helpers read entity ids by direct field access
(`${entityParamName}.pets` → `pet -> pet.id`), because Panache entities have public
fields and no `getId()` to point a method reference at. See
`examples/_fragments/flat-expression/java.md` for the exact body.