...
## Insert Point
As constructor(s) in the class body, after fields.
...
// emitted. Generate both even if the user did not explicitly ask for the
// no-args one.
...
- Constructor parameters all on a single line — no per-parameter wrapping, even for many fields.
...
| `${TypeN}` | field types from the selected entity attributes | -- |