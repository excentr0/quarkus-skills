...
i.e. for each selected field a `get` then a `set` then a blank line, in the same order as the fields.
There is no "all getters first, then all setters" section.
...
## Formatting rules
- **Multi-line**: signature on its own line, body indented one level, closing `}` on its own line.
  Never collapse to one line.
- **No blank line** between a getter and its paired setter; **one blank line** before the next field's getter.
